      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-6 pb-12">
        ...
        {results && searchValues && (
          <div ref={resultsRef} className="mt-2">
            {/* Agoda loading indicator — shown while background fetch is running */}
            {agodaUpdating && (
              <div className="flex items-center gap-2 text-xs text-gray-400 mb-3 px-1">
                <svg className="w-3.5 h-3.5 animate-spin text-teal flex-shrink-0" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth={4} />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                </svg>
                Fetching Agoda prices in the background…
              </div>
            )}
            {agodaPrices && !agodaUpdating && (
              <div className="flex items-center gap-1.5 text-xs text-teal mb-3 px-1">
                <span>✓</span> Agoda prices loaded — cards updated with best deal
              </div>
            )}
            <HotelGrid
              initialData={results}
