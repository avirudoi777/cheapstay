  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 mt-6 pb-12">
        ...
        {/* Search source status — shown while loading or while Agoda is fetching */}
        {(loading || bookingDone) && (
          <div className="flex items-center gap-3 my-4 flex-wrap">
            {/* Booking.com pill */}
            <div className={`flex items-center gap-2 px-3.5 py-2 rounded-full text-xs font-semibold border transition-all duration-300 ${
              bookingDone
                ? 'bg-teal/8 border-teal/30 text-teal'
              ...
              Booking.com
              {bookingDone && bookingCount > 0 && (
                <span className="opacity-70 font-normal">· {bookingCount} hotels</span>
              )}
            </div>
            ...
            <div className={`flex items-center gap-2 px-3.5 py-2 rounded-full text-xs font-semibold border transition-all duration-300 ${
              ...
              Agoda
              {agodaUpdating && <span className="opacity-70 font-normal">· fetching…</span>}
              {agodaPrices && !agodaUpdating && <span className="opacity-70 font-normal">· prices merged</span>}
              {!agodaUpdating && !agodaPrices && bookingDone && <span className="opacity-50 font-normal">· pending</span>}
            </div>
          </div>
        )}
